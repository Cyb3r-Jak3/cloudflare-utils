...
__cloudflare-utils_init_completion() {
  COMPREPLY=()
...
__cloudflare-utils_build_completion_request() {
...
__cloudflare-utils_find_description_for() {
  local candidate="$1"
  local i
...
__cloudflare-utils_bash_autocomplete() {
  ...
  if [[ "${words[0]}" != "source" ]]; then
    ...
    __cloudflare-utils_init_completion -n "=:" || return

    request_comp="$(__cloudflare-utils_build_completion_request)"
    ...
    if [[ "${COMP_TYPE:-}" == "63" && ${#matches[@]} -gt 0 ]]; then
      ...
      for candidate in "${matches[@]}"; do
        local desc="$(__cloudflare-utils_find_description_for "${candidate}")"
...
complete -o bashdefault -o default -F __cloudflare-utils_bash_autocomplete cloudflare-utils
